...
pos.LS.ESS=function(y,x,N1,M,nu,l,eta,mcmc,brn,thin,tau.in,sig.in,xi.in,prior,
  ...
  # prior: Choose between methods for generating the prior: either using 'Fast.LS' or 'LS.KLE'
  ...
  n=length(y)
  N <- N1*M
  delta=1/(N-1)#(max(x)-min(x))/(N-1)
  my_knots=seq(0,1,by=delta)#seq(min(x),max(x),by=delta)
...
pos.WC.ESS=function(y,x,N,nu,l,eta,mcmc,brn,thin,tau.in,sig.in,xi.in,
  ...
  if(missing(xi.fix) && missing(xi.in)){
    ...
    z_star.in <- solve.QP(XXK,dvec=as.vector(t(X)%*%y)/sig.in,
                          Amat=t(Amat),bvec=c(rep(0,N)),meq=0)$solution
    xi.in=z_star.in#pmax(0,mvrnorm(1,rep(0,N),K))#
  }
  tau=tau.in
  sig=sig.in
  xi_in=xi.in
  ...
  for(i in 1:em){
    # sampling Xi:
    if(missing(xi.fix)){
      nu.ess = as.vector(samp.WC(my_knots,nu,l,tau,sseedWC=i))
      xi_out = ESS(xi_in,nu.ess,y,X,sig,eta,seeds=i)
      xi_out = sapply(xi_out,function(z) return(pmax(0,z)))
    }else{
      xi_out=xi_in
    }
...
########### For nonnegative functions estimation ##############
pos.LS.ESS_hyp=function(y,x,N1,M,nu.in,l.in,eta,mcmc,brn,thin,tau.in,sig.in,xi.in,prior,
                        tau.fix,sig.fix,xi.fix,sseed,verbose,return.plot,tol){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N1: number of knots first subdomain; M: nb of subdomain
  # eta:parameter of the approximation function of the indicator functions
  ...
  if(missing(nu.in))
    nu.in=0.75
  if(missing(l.in))
    l.in=l_est(nu.in,range=range(my_knots),0.05)
  ...
  for(i in 1:em){
    ...
    if(missing(xi.fix)){
      if(prior=='Fast.LS'){
        nu.ess = Fast.LS(my_knots,M=M,N1=N1,nu=nu_out,l=l_out,tausq=tau,tol=tol,sseedLS=i)
      }
      if(prior=='LS.KLE'){
        nu.ess = LS.KLE(my_knots,M=M,N1=N1,nu=nu_out,l=l_out,tausq=tau,tol=tol,sseedLS=i)
      }
      xi_out = ESS(xi_in,nu.ess,y,X,sig,eta,seeds=i)
      xi_out = sapply(xi_out,function(z) return(pmax(0,z)))
    }else{
...
pos.HMC=function(y,x,N,nu,l,mcmc,brn,thin,tau.in,sig.in,xi.in,
  ...
  for(i in 1:em){
    ...
    if(missing(xi.fix)){
      ...
      xi_out = as.vector(rtmg(1,M,r,xi_in,f,g,burn.in=0))
    }else{
      xi_out=xi_in
    }
    set.seed(2*i)
...
mon.inc.LS.ESS=function(y,x,N1,M,nu,l,eta,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi.in,xi0.fix,prior,
  ...
  for(i in 1:em){
    ...
    if(missing(xi.fix)){
      if(prior=='Fast.LS'){
        nu.ess = Fast.LS(my_knots,M=M,N1=N1,nu=nu,l=l,tausq=tau,tol=tol,sseedLS=i)
      }
      if(prior=='LS.KLE'){
        nu.ess = LS.KLE(my_knots,M=M,N1=N1,nu=nu,l=l,tausq=tau,tol=tol,sseedLS=i)
      }
      xi_out = ESS(xi_in,nu.ess,y=y_tilde,X,sig,eta,seeds=i)
      xi_out = sapply(xi_out,function(z) return(pmax(0,z)))
    }else{
      xi_out=xi_in
    }
  ...
  K[1,2:(N+1)] <- rep(0,N)
  K[2:(N+1),1] <- rep(0,N)
  K[2:(N+1),2:(N+1)] = covmat(my_knots,nu,l)
  K_invp = bdiag(1/K[1,1],K_inv)#solve(K)
  XXK <- crossprod(Xp)/mean(sig_sam)+K_invp/mean(tau_sam)
  Amat <- diag(N+1)
  Amat <- Amat[-1,]
  z_star <- solve.QP(XXK,dvec=as.vector(t(Xp)%*%y)/mean(sig_sam),
                     Amat=t(Amat),bvec=c(rep(0,N)),meq=0)$solution
...
mon.inc.WC.ESS=function(y,x,N,nu,l,eta,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi.in,xi0.fix,
  ...
  for(i in 1:em){
    ...
    y_tilde = y - xi0
    if(missing(xi.fix)){
      nu.ess = as.vector(samp.WC(my_knots,nu=nu,l=l,tausq=tau,sseedWC=i))
      xi_out = ESS(xi_in,nu.ess,y=y_tilde,X,sig,eta,seeds=i)
      xi_out = sapply(xi_out,function(z) return(pmax(0,z)))
    }else{
      xi_out=xi_in
...
######### For monotone nondecreasing functions estimation ##############
mon.HMC=function(y,x,N,nu,l,mcmc,brn,thin,tau.in,sig.in,xi.in,xi0.in,
  ...
  for(i in 1:em){
    ...
    if(missing(xi.fix)){
      ...
      xi_out = as.vector(rtmg(1,M=M,r=r,initial=xi_in,f=f,g=g,burn.in=0))
    }else{
      xi_out=xi_in
    }
    set.seed(2*i)
...
### Function for drawing posterior samples using Gibbs sampler with fixed hyperparameters:

######### For monotone nondecreasing functions estimation ##############
mon.Gibbs=function(y,x,N,nu,l,mcmc,brn,thin,tau.in,sig.in,xi.in,xi0.in,
                   tau.fix,sig.fix,xi.fix,xi0.fix,sseed,verbose,return.plot,tol){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N: number of knots 
  # nu:smoothness parameter of Matern; l:length-scale parameter of Matern
  ...
  for(i in 1:em){
    ...
    # sampling Xi:
    if(missing(xi.fix)){
      constr = thresh2constraints(d=N,lower=rep(0,N))
      # M = chol2inv(chol(crossprod(X)/sig + K_inv/tau))
      # r = as.vector(t(X)%*%y_tilde)/sig
      invXKX <- chol2inv(chol(X%*%K%*%t(X)+sig*diag(n)))
      cov <- K-K%*%t(X)%*%invXKX%*%X%*%K
      zcentre <- K %*%t(X)%*%invXKX%*%y
      
      xi_out = as.vector(
        sample_from_constraints(linear_part=constr$linear_part,
                                offset=constr$offset,mean_param=zcentre,covariance=cov,
                                initial_point=rep(1,N),ndraw=1,burnin=0)
      )
      xi_out = sapply(xi_out,function(z) return(pmax(0,z)))
    }else{
      xi_out=xi_in
...
### Function for drawing posterior samples using ESS and LS with hyperparameter updates:
## monotone increasing functions estimation

########### For monotone functions estimation ##############
mon.inc.LS.ESS_hyp=function(y,x,N1,M,nu.in,l.in,eta,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi.in,xi0.fix,prior,
                            tau.fix,sig.fix,xi.fix,verbose,return.plot,tol,sseed){
  ...
  K[1,2:(N+1)] <- rep(0,N)
  K[2:(N+1),1] <- rep(0,N)
  K[2:(N+1),2:(N+1)] = covmat(my_knots,nu=mean(nu_sam),l=mean(ell_sam))
  K_inv=solve(K)
  XXK <- crossprod(X)/mean(sig_sam)+K_inv/mean(tau_sam)
  Amat <- diag(N+1)
  Amat <- Amat[-1,]
...
mon.inc.WC.ESS_hyp=function(y,x,N,nu.in,l.in,eta,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi.in,xi0.fix,
  ...
  for(i in 1:em){
    ...
    y_tilde = y - xi0
    if(missing(xi.fix)){
      nu.ess = samp.WC(my_knots,nu=nu_out,l=l_out,tausq=tau,sseedWC=i)
      xi_out = ESS(xi_in,nu.ess,y=y_tilde,X,sig,eta,seeds=i)
      xi_out = sapply(xi_out,function(z) return(pmax(0,z)))
    }else{
      xi_out=xi_in
...
########### For decreasing function estimation ##############
mon.dec.LS.ESS_hyp=function(y,x,N1,M,nu.in,l.in,eta,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi.in,xi0.fix,prior,
                            tau.fix,sig.fix,xi.fix,verbose,return.plot,tol,sseed){
  # y:Response variable; x: vector to form design matrix \phi (n X N)
  # N1: number of knots first subdomain; M: nb of subdomain
  # eta:parameter of the spproximation function of the indicator functions
  ...
  for(i in 1:em){
    ...
    if(missing(xi.fix)){
      ...
      xi_out = ESS.dec(xi_in,nu.ess,y=y_tilde,X,sig,eta,seeds=i)
      xi_out = sapply(xi_out,function(z) return(pmin(0,z)))
    }else{
      xi_out=xi_in
    }
    set.seed(2*i)
...
### Function for drawing posterior samples using ESS and LS with fixed hyperparameter:

########### For decreasing function estimation ##############
mon.dec.LS.ESS=function(y,x,N1,M,nu,l,eta,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi.in,xi0.fix,prior,
                        tau.fix,sig.fix,xi.fix,verbose,return.plot,tol,sseed){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N1: the number of knots of the first subdomain
  # M: nb of subdomain
  ...
  if(missing(xi0.fix) && missing(xi0.in))
    xi0.in=0
  if(missing(xi.fix) && missing(xi.in))
    xi.in=pmin(0,mvrnorm(1,rep(0,N),K))
...
######### Run-time / iteration ####################
###################################################
## functions without the MAP estimate 
...
###################################################
## LS-ESS sampler
mon.inc.LS.ESS_1iter=function(y,x,N1,M,nu,l,eta,mcmc,brn,thin,tau.in,sig.in,xi0.in,xi.in,xi0.fix,
                              tau.fix,sig.fix,xi.fix,verbose,tol,sseed){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N1: the number of knots of the first subdomain
  # M: nb of subdomain
  # nu:smoothness parameter of Matern; l:length-scale parameter of Matern
...
######### For monotone nondecreasing functions estimation ##############
## HMC sampler
mon.HMC_1iter=function(y,x,N,nu,l,mcmc,brn,thin,tau.in,sig.in,xi.in,xi0.in,
                       tau.fix,sig.fix,xi.fix,xi0.fix,sseed,verbose,tol){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N: number of knots 
  # nu:smoothness parameter of Matern; l:length-scale parameter of Matern
  # mcmc, brn, thin : mcmc samples, burning and thinning for MCMC
...
###################################################
###### Nonnegative function estimation #############
###################################################

######### HMC sampler 
pos.HMC_1iter=function(y,x,N,nu,l,mcmc,brn,thin,tau.in,sig.in,xi.in,
                       tau.fix,sig.fix,xi.fix,sseed,verbose,tol){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N: the number of knots
  # nu:smoothness parameter of Matern; l:length-scale parameter of Matern
  # mcmc, brn, thin : mcmc samples, burning and thinning for MCMC
...
## LS-ESS
pos.LS.ESS_1iter=function(y,x,N1,M,nu,l,eta,mcmc,brn,thin,tau.in,sig.in,xi.in,
                          tau.fix,sig.fix,xi.fix,sseed,verbose,tol){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N1: number of knots first subdomain; M: nb of subdomain
  # nu:smoothness parameter of Matern; l:length-scale parameter of Matern
  ...
  for(i in 1:em){
    # sampling Xi:
    if(missing(xi.fix)){
      nu.ess = Fast.LS(my_knots,M,N1,nu=nu,l=l,tausq=tau,tol=tol,sseedLS=i)
      xi_out = ESS(xi_in,nu.ess,y,X,sig,eta,seeds=i)
      xi_out = sapply(xi_out,function(z) return(pmax(0,z)))
    }else{
      xi_out=xi_in
...
## Gibbs sampler 
pos.Gibbs_1iter=function(y,x,N,nu,l,mcmc,brn,thin,tau.in,sig.in,xi.in,
                         tau.fix,sig.fix,xi.fix,sseed,verbose,return.plot,tol){
  # y:Response variable; x: vector to form design matrix X (n x N)
  # N: the number of knots
  # nu:smoothness parameter of Matern; l:length-scale parameter of Matern
  ...
  ## posterior Mode
  XXK <- crossprod(X)/sig+K_inv/tau
  Amat <- diag(N)
  z_star <- solve.QP(XXK,dvec=as.vector(t(X)%*%y)/sig,
                     Amat=t(Amat),bvec=c(rep(0,N)),meq=0)$solution
  # xi_in=z_star
  ptm=proc.time()
  for(i in 1:em){
    ...
    if(missing(xi.fix)){
      ...
      constr = thresh2constraints(N,lower=rep(0,N))
      xi_out = as.vector(
        sample_from_constraints(linear_part=constr$linear_part,
                                offset=constr$offset,mean_param=M%*%r,covariance=M,
                                initial_point=z_star,ndraw=1,burnin=10)
      )
      xi_out=pmax(1e-4,xi_out)
    }else{
      xi_out=xi_in
    }
    set.seed(2*i)
  ...
  return(list("time"=tm,"xi_sam"=xi_sam,"sig_sam"=sig_sam,"tau_sam"=tau_sam,
              "fhat_sam"=fhat_sam,"fmean"=fmean,"f_low"=f_low,"f_upp"=f_upp,"z_star"=z_star,"A"=X))
}
####################################################
