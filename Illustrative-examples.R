...
## the section `Numerical performance' of the article
## Comparison between LS and Cholesky
## Comparison between LS and FFT Wood and Chan in different situations
...
if (WCvsLS_nu == 'yes') {
  ...
  par(mar = c(3.1, 3.1, 1.1, 1.1)) # adapt margins
  nu <- seq(from = 0.5, to = 1.5, length = 10) # smoothness parameter Matern Kernel (MK)
...
if (WCvsLS_theta == 'yes') {
  ...
  theta <- seq(from = 0.1, to = 1, length = 10) # length-scale parameter MK
  N <- 10000 # sizes of MVN
  N1 <- 100 # size of 1st subdomain
  M <- N/N1 # nb of subdomains
  trial <- 50 # nb of replicates
  timeWC <- matrix(NA, nrow = length(theta), ncol = trial)
  timeLS <- matrix(NA, nrow = length(theta), ncol = trial)
  u <- seq(from = 0, to = 1, length = N)
...
if (LSvsLS.WC_N == 'yes') {
  ...
  M <- N/N1 # nb of subdomains
  trial <- 25 # nb of replicates
  timeLS.WC <- matrix(NA, nrow = trial, ncol = length(N))
  timeLS <- matrix(NA, nrow = trial, ncol = length(N))
  for (j in 1 : trial) {
    print(j)
    for (i in 1 : length(N)) {
...
if (LS1M == 'yes') {
  ...
  M <- seq(from = 500, to = 2000, length = 11) # nbs of blocks (left panel)
...
if (correlation == 'yes') {
  ...
  corLS <- matrix(NA, nrow = N/2, ncol = trial)
  corWC <- matrix(NA, nrow = N/2, ncol = trial)
  for (j in 1 : trial) {
    print(j)
...
if (monotoneC0Approx == 'yes') {
  ...
  delta <- 1/(N-1)
  l <- 500 # les pas de x
  u <- seq(from = 0, to = 1, by = delta) # discretized input set
  f <- function(x) {
    x^3#exp(2*x)/6
  }
  x <- seq(from = 0, to = 1, length = l)
...
if (comp_pos_syn_LS_WC == 'yes') {
  ...
  for (Q in 1 : trial) {
    ...
    post_syn_LS <- pos.LS.ESS(y = ytr, x = xtr, N1 = N1, M = M, eta = 100,
                              mcmc = nbsim, brn = brn, thin = 1,
                              nu = nu, l = l, tau.in = 1,
                              sig.in = sigN^2,
                              sseed = Q, return.plot = F, tol = tol)
    post_syn_WC <- pos.WC.ESS(y = ytr, x = xtr, N = N, eta = 100,
                              mcmc = nbsim, brn = brn, thin = 1,
                              nu = nu, l = l, tau.in = 1,
                              sig.in = sigN^2,
                              sseed = Q, return.plot = F, tol = tol)
  ...
  plot(xtr, ytr, pch = '*', lwd = 2, lty = 1, col = 'black',
       ylim = range(f_low_LS, f_upp_LS, f_low_WC, f_upp_WC, ytr), xlab = '', ylab = '')
  ...
  plot(xtr, ytr, pch = '*', lwd = 2, lty = 1, col = 'black',
       ylim = range(f_low_WC, f_upp_WC, f_low_LS, f_upp_LS, ytr), xlab = '', ylab = '')
...
if (comp_monot_syn_LS_WC == 'yes') {
  ...
  plot(xtr, ytr, pch = '*', lwd = 2, lty = 1, col = 'black',
       ylim = range(f_low_LS, f_upp_LS, f_low_WC, f_upp_WC, ytr, MAP_LS), xlab = '', ylab = '')
  polygon(c(t, rev(t)), y = c(f_low_LS, rev(f_upp_LS)), border = F, col = 'gray')
  lines(t, f(t), lwd = 2, lty = 1, col = 'black')
  lines(t, fmean_LS, type = 'l', lty = 4, lwd = 2, col = 'blue')
