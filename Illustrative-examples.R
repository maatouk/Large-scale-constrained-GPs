...
comp_monot_HMC_LS='' # run-time/iteration
comp_pos_syn_HMC_LS='' # run-time/iteration
...
if(comp_monot_HMC_LS=='yes'){
  ...
  for(i in 1 : length(n)){
    ...
    for(Q in 1 : trial){
      ...
      post_mon_syn_LS <- mon.inc.LS.ESS_1iter(y,x,N1=N1,M=M,nu=nu,l=l,eta=50,mcmc=nbsim,
                                              brn=brn,thin=1,sig.in=sigN^2,tau.in=1,
                                              sseed=Q,tol=tol)
      post_mon_syn_HMC <- mon.HMC_1iter(y,x,N=N,nu=nu,l=l,mcmc=nbsim,
                                        brn=brn,thin=1,sig.in=sigN^2,tau.in=1,
                                        sseed=Q,tol=tol)
  ...
  legend(1000,1,c('HMC','LS-ESS'),col='black',
...
if(comp_pos_syn_HMC_LS=='yes'){
  ...
  brn <- 0#1000 # burn in
  tol <- 1e-6
  nu <- 1.5 # smooth parameter Matern kernel
  l <- l_est(nu,c(0,1),0.05) # length-scale
  f <- function(x){
    ## Hassan postive function
    #4*x+(cos(10*x))-0.1
  ...
  for(i in 1 : length(n)){
    ...
    for(Q in 1 : trial){
      ...
      post_pos_syn_LS <- pos.LS.ESS_1iter(y,x,N1=N1,M=M,nu=nu,l=l,eta=50,mcmc=nbsim,
                                          brn=brn,thin=1,sig.in=sigN^2,tau.in=1,
                                          sseed=Q,tol=tol)
      post_pos_syn_HMC <- pos.HMC_1iter(y,x,N=N,nu=nu,l=l,mcmc=nbsim,
                                        brn=brn,thin=1,sig.in=sigN^2,tau.in=1,
                                        sseed=Q,tol=tol)
      timeLS[Q,i] <- post_pos_syn_LS$time[3]
      timeHMC[Q,i] <- post_pos_syn_HMC$time[3]
    }
  }
  ## Illustration
  par(mfrow=c(1,1))
  ...
  lines(n,colMeans(timeHMC),type='l',lwd=2,lty=2)
  title(xlab='sample size (n)',ylab='Run-time/iteration (s)',line=2)
  mtext(text='nonnegativity constraints',line=0.3)
  legend(1000,1,c('HMC','LS-ESS'),col='black',
         lwd=2,lty=c(2,1),text.font=1,box.lty=0,cex=1,
         bg='transparent')
  #############################################
  ...
  G <- G  + theme_bw()
  Glabs <- G+labs(title='nonnegativity constraints',
                  x='sample size (n)',
                  y='Run-time/iteration (s)')+
    theme(plot.title = element_text(hjust = 0.5, size=10),
          plot.subtitle = element_text(hjust = 0.5,size=10))
